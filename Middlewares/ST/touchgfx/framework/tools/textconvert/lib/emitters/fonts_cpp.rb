...
class FontsCpp
  ...
  def initialize(text_entries, typographies, languages, output_directory, font_asset_path, autohint_setting, data_format, generate_binary_fonts, generate_font_format, korean_fusion_font)
  ...
  def run
    ...
    unique_typographies.sort_by { |t| sprintf("%s %04d %d",t.font_file,t.font_size,t.bpp) }.each do |typography|
      ...
      fusion = "#{typography.cpp_name}_#{typography.font_size}_#{typography.bpp}bpp" == @korean_fusion_font ? 1 : 0
      cmd = "\"#{@@font_convert}\" \
-f \"#{font_file}\" \
-i #{font_index} \
