...
static uint32_t OSPI_TestCode(uint32_t address);
...
void HAL_OSPI_MspDeInit(OSPI_HandleTypeDef *ospiHandle)
{
  ...
  if (ospiHandle->Instance == OCTOSPI1)
  {
    ...
    PA1     ------> OCTOSPIM_P1_DQS
    */
    HAL_GPIO_DeInit(GPIOF, GPIO_PIN_10 | GPIO_PIN_7 | GPIO_PIN_9 | GPIO_PIN_6 | GPIO_PIN_8);
...
uint8_t aTxBuffer[] = "*** STM32U5A9J-DK OSPI Memory-Mapped Mode ***";
static uint32_t OSPI_TestCode(uint32_t address)
{
  ...
  /* Write Test */
  memAddr = (uint8_t *)(OCTOSPI1_BASE + address);
  for(index = 0; index < BUFFERSIZE; index++)
  {
    *memAddr = aTxBuffer[index];
    memAddr++;
  }
